...
# ── KPI Cards ─────────────────────────────────────────────────────────────────
income_total  = trx_sector[trx_sector["type"] == "income"]["amount"].sum()
expense_total = trx_sector[trx_sector["type"] == "expense"]["amount"].sum()
...
sector_income  = trx_sector[trx_sector["type"] == "income"].groupby("sector")["amount"].sum()
sector_expense = trx_sector[trx_sector["type"] == "expense"].groupby("sector")["amount"].sum()
...
ax.plot(monthly["tahun_bulan_dt"], monthly.get("income", pd.Series(dtype=float)) / 1e6,
        label="Pendapatan", color=PRIMARY, linewidth=2, marker="o", markersize=3)
ax.plot(monthly["tahun_bulan_dt"], monthly.get("expense", pd.Series(dtype=float)) / 1e6,
        label="Pengeluaran", color=DANGER, linewidth=2, marker="o", markersize=3)
ax.fill_between(monthly["tahun_bulan_dt"], monthly["net_profit"] / 1e6, 0,
                where=monthly["net_profit"] >= 0, alpha=0.15, color=SUCCESS, label="Laba Bersih (+)")
ax.fill_between(monthly["tahun_bulan_dt"], monthly["net_profit"] / 1e6, 0,
                where=monthly["net_profit"] < 0, alpha=0.15, color=DANGER, label="Laba Bersih (-)")
...
if len(monthly) >= 2:
    ...
    prev   = monthly.iloc[-2]
    income_growth = ((latest.get("income", 0) - prev.get("income", 0)) / prev.get("income", 1) * 100)
    st.info(
...
income_time = trx_sector[trx_sector["type"] == "income"].copy()
